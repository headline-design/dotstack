...
export const Profile: React.FC<Props> = ({ account, token, onSignOut }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <div className="text-muted-foreground text-xs">{a0id ?? truncateMiddle(account.address, 5, 5, "...")}</div>
          </div>
        </div>
      ...
      <p className="text-stone-500 text-center text-sm mt-auto">
        Click{" "}
        <span className="text-foreground cursor-pointer hover:text-stone-400" onClick={() => generate()}>
          here
        </span>{" "}
